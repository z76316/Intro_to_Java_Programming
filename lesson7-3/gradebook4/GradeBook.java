...
public class GradeBook
{
    ...
    public String getTeacherComment(int studentIndex)
    {
        ...
        double oneTopic = 0;
        double average = 0;        
        double great = 0; double good = 0; double oops = 0;
        for(int topic = 0; topic < numTopics; topic++)
        {
            oneTopic = grades[studentIndex][topic];
            total = total + oneTopic;
            if(oneTopic >= 9)
            {
                great++;
            }
        ...
        average = total / numTopics;
        String aboutAverage = ""; 
        if(average >= 9)
        {
            aboutAverage = "This guy's grade is great!";
        }
        else if(average >= 6)
        {
            aboutAverage = "This guy's grade is just fine.";
        }
        else
        {
            aboutAverage = "This guy's grade...please.";
        }
        String abouTtopic = "";
        if(great / numTopics >= 0.7)
        {
            abouTtopic = "At the same time, almost has great grade in every topics.";
        }
        else if(great / numTopics + good / numTopics >= 0.7)
        {
            abouTtopic = "At the same time, can handle lots of topic.";
        }
        else if(good / numTopics >= 0.7)
        {
            abouTtopic = "At the same time,almost has okay performance in every topic";
        }
        else
        {
            abouTtopic = "You should...you know. If not...hey hey hey.";
        }
        return aboutAverage + " " + abouTtopic;
    }
